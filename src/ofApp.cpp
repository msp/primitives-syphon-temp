...
void ofApp::setupMIDI(){
    ...
    // add the ofApp as a listener to allow preset selection on the soundbank
    midiIn.addListener(this);
...
bool ofApp::isFireMIDI(ofxMidiMessage& msg){
    bool ret = false;
...
void ofApp::setPreset(int _preset) {
    for(int i = 0; i<soundBank.activeSlots.size(); i++) {
        midiIn.removeListener(soundBank.activeSlots.at(i));
    }
...
bool ofApp::isRunningStandalone(){
    string sTrue = "true";
